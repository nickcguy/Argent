package net.ncguy.argent.editorOld.panels.menu;
...
public class ObjectContextMenu<T extends WorldEntity> {
    ...
    protected void init() {
        ...
        rootMenu.addItem(addObjectItem = new MenuItem("Add object", new ChangeListener() {
            @Override public void changed(ChangeEvent event, Actor actor) { addObject(); }
        }));
        rootMenu.addItem(dupeObjectItem = new MenuItem("Clone object", new ChangeListener() {
            @Override public void changed(ChangeEvent event, Actor actor) { dupeObject(); }
        }));
        rootMenu.addItem(delObjectItem = new MenuItem("Delete object", new ChangeListener() {
            @Override public void changed(ChangeEvent event, Actor actor) { delObject(); }
        }));

        this.dupeObjectItem.setDisabled(true);
        this.delObjectItem.setDisabled(true);

    }
    private void addObject(T instance) {
        this.editor.world().addInstance(instance);
        this.editor.select(instance);
    ...
    public void dupeObject() {
        addObject((T) target.copy());
    }
    ...
    public void target(T selected) {
        ...
        this.dupeObjectItem.setDisabled(this.target == null);
    }
